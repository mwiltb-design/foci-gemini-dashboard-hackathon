...
export function FileBrowser({ workspaceRevision, editable }: { workspaceRevision: number; editable: boolean }) {
  ...
  return (
    <Panel
      ...
      action={<div className="file-panel-actions">{editable && <button className="button button--quiet" type="button" onClick={() => setCreating((value) => !value)}>＋ New file</button>}<button className="button button--quiet" type="button" onClick={files.refresh}>↻ Refresh</button></div>}
      fullWidth
    >
