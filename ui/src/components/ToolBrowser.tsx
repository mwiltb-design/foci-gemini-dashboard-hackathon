...
export function ToolBrowser({ revision, mode }: { revision: number; mode: ToolCatalogMode }) {
  ...
  return (
    <Panel eyebrow="Live Gemini runtime catalog" title={active ? 'Active Tools' : 'Available Tools'} action={<button className="button button--quiet" type="button" onClick={browser.refresh}>↻ Refresh</button>} fullWidth>
      <div className="panel__body">
        <div className="metrics">
          <div className="metric"><b>{browser.tools.length}</b><span>{active ? 'Exposed to Gemini' : 'Not exposed to Gemini'}</span></div>
          <div className="metric"><b>{active ? activeCount : dependencyCount}</b><span>{active ? 'Gemini has access' : 'Plugin dependencies'}</span></div>
        ...
        <div className="tools-explainer">
          <strong>{active ? 'These tools are exposed to Gemini in the current runtime.' : 'These tools are known but not currently exposed to Gemini.'}</strong>
          <span>{active ? 'The live Gemini snapshot is authoritative.' : 'Plugin tools show their enablement and Gemini-access dependency; built-in tool toggles remain unavailable pending a separate safety review.'}</span>
        </div>
        <div className={`tools-browser ${active ? 'tools-browser--without-shell' : ''}`}>
            ...
            <div className="tools-list">
              ...
              {!browser.loading && browser.filtered.map((tool) => (
                <button className={`runtime-tool-row ${browser.selectedName === tool.name ? 'is-selected' : ''}`} type="button" key={tool.name} onClick={() => browser.setSelectedName(tool.name)}>
                  ...
                  <span><strong>{tool.name}</strong><em>{tool.description}</em><small>{accessLabels[tool.access]} · {tool.source} · {tool.piAccess ? 'Gemini access' : 'No Gemini access'}</small></span>
            ...
            {!browser.selected ? <div className="tool-empty tool-empty--large">Select a tool to inspect its access and runtime source.</div> : (
              <>
                <header className="tool-detail-head">
                  ...
                  <Chip tone={browser.selected.piAccess ? 'accent' : 'neutral'}>{browser.selected.piAccess ? 'Gemini has access' : 'Gemini has no access'}</Chip>
                </header>
                <div className="tool-detail-body">
                  ...
                  <section className="tool-status">
                    ...
                    {browser.selected.dependency && <p>Dependency: {browser.selected.dependency.name} plugin · {browser.selected.dependency.enabled ? 'enabled' : 'disabled'} · Gemini {browser.selected.dependency.access} access {browser.selected.dependency.granted ? 'granted' : 'not granted'}.</p>}
                  </section>
                  ...
                  <div className={`tool-safety-note tool-safety-note--${browser.selected.risk}`}>
                    ...
                    {browser.selected.access === 'execute' && 'This tool can run installed command-line programs. Its impact depends on the command Gemini executes.'}
          ...
          {!active && <aside className="shell-capabilities">
            <header><span className="eyebrow">Indirect capabilities</span><h2>Command-line programs</h2><p>These are not direct Gemini tools. Their status is reported separately because Bash may expose them indirectly.</p></header>
            <div className="shell-list">{browser.shell.map((program) => (
              <div className={!program.available ? 'is-unavailable' : ''} key={program.name}>
                ...
                <span><strong>{program.label}</strong><em>{program.description}</em><small>{program.source} · {program.piAccess ? 'Gemini access through Bash' : 'No Gemini access'}<br />{program.status}<br />{program.version ?? 'version unavailable'}</small></span>
              </div>
            ))}</div>
          </aside>}
        </div>
